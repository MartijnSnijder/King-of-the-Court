...
function startRound(round) {
    ...
    if (round == 1) {
        ...
        alert('Team ' + (eliminate[0] + 1) + ' (' + info[eliminate[0]]['name'] + ') ligt er uit!');
    } else if (round == 3) {
        currentTime = -1;
        var eliminate = eliminateTeam(score);
        score = eliminate[1]; //reset to 0
    ...
    $('#roundnr').html('Ronde ' + round);
    setHtmlRanking();
    setHtmlOrder();

    alert('Zodra u deze popup sluit, begint de ronde!');
    startTimer();
}
...
function setHtmlRanking() {
    ...
    for (var i = 0; i < masterList.length; i++) {
        ...
        $('#nr' + (i)).html('<td style="background-color: ' + bgcolor + '; color: ' + color + '">' + name + '</td><td>' + points + '</td>')
    }
}
...
function awardPoint() {
    ...
    if (score[winnerssideTeam] == 15 && round == 3) {
        alert('Team ' + (winnerssideTeam + 1) + ' (' + info[winnerssideTeam]['name'] + ') heeft gewonnen!');
    }
    setHtmlRanking();
}
